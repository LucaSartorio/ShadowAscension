extends SceneTree

## M12.6 — the support archetype through the whole game.
##
##   godot --headless --path . --script res://tests/core/m12_support_run.gd
##
## The shipped dungeon holds melee only: placing supports in it is content
## (M18). This run brings its own into the real dungeon, on its real navigation:
##
## Menu -> New Game -> hub (a shadow summoned) -> gate -> dungeon: the shipped
## melee untouched -> room one, a support beside its two melee: one of them
## brought to 40%, and the support heals it (40 -> 65) through its health, its
## bar redrawn; hurt again, the next heal is cut off by the player's heavy; the
## melee killed, the room clears; locked on, a critical heavy brings the support
## down: 30 XP once, the lock let go -> room two: a support the shadow finishes
## (70/30) -> the boss, untouched -> leaving while a support is casting a heal:
## nothing of it survives the scene change -> hub -> a second dungeon: the support
## fresh, no listener doubled, the same heal again -> hub, nothing orphaned.

const BOOT: String = "res://Main.tscn"
const HUB: String = "res://scenes/core/hub.tscn"
const SUPPORT_SCENE: PackedScene = preload("res://scenes/enemies/basic_support_enemy.tscn")
const MELEE_SCENE: PackedScene = preload("res://scenes/enemies/basic_melee_enemy.tscn")
const HEAL: AttackData = preload("res://resources/enemies/attacks/support_heal.tres")
const DT: float = 1.0 / 60.0
const ROOM_ANCHORS: Array[Vector3] = [
	Vector3(0, 0.1, -13), Vector3(0, 0.1, -33), Vector3(0, 0.1, -53)
]
const ROOM_ONE_SUPPORT: Vector3 = Vector3(-4, 0.1, -20)
const ROOM_TWO_SUPPORT: Vector3 = Vector3(4, 0.1, -41)
const EXIT_POS: Vector3 = Vector3(0, 0.1, -65)
const STRIKE_RANGE: float = 1.6

var _no_crits: PlayerCombatData = preload("res://resources/characters/player_combat.tres")
var _shadow_data: ShadowData = preload("res://resources/shadows/basic_melee_shadow.tres")
var _state: Node = null
var _pass: int = 0
var _fail: int = 0
## Every support action that landed: {support, ally, action, amount}.
var _applied: Array[Dictionary] = []
var _first_listeners: Array = []


func _initialize() -> void:
	_no_crits.critical_chance = 0.0
	_state = root.get_node_or_null("PlayerRuntimeState")
	_state.reset_runtime_state()
	change_scene_to_file(BOOT)
	await _pause(0.6)
	(current_scene.get_node("MainMenu") as MainMenu).press_play()
	await _pause(1.4)
	await _phase_hub()
	await _phase_into_dungeon(1)
	await _phase_room_one(1)
	await _phase_room_two()
	await _phase_boss()
	await _phase_out_mid_cast()
	await _phase_into_dungeon(2)
	await _phase_room_one(2)
	SceneTransition.find_in(self).transition_to_scene(HUB)
	await _pause(1.6)
	_record(current_scene.scene_file_path == HUB and _supports_anywhere() == 0
			and int(Performance.get_monitor(Performance.OBJECT_ORPHAN_NODE_COUNT)) == 0,
		"END) back in the hub after the second dungeon: no support left anywhere, nothing orphaned")
	print("[SUMMARY] passed=%d failed=%d" % [_pass, _fail])
	await _pause(0.3)
	quit()


# --- the hub -------------------------------------------------------------------------------------------

func _phase_hub() -> void:
	var p: Player = current_scene.get_node("Player")
	var shadow: ShadowInstance = p.shadows.add_shadow(_shadow_data)
	p.shadow_summoner.summon(shadow.instance_id)
	await _pause(0.4)
	p.shadow_commander.set_mode(BasicMeleeShadow.CommandMode.FOLLOW)
	_record(p.shadow_summoner.get_active_node() != null, "H1) the hub: a shadow summoned, to take into the dungeon")


func _phase_into_dungeon(run: int) -> void:
	var p: Player = current_scene.get_node("Player")
	var gate: DungeonGate = current_scene.get_node("DungeonGate")
	p.global_position = gate.global_position
	await _pause(0.4)
	gate.activate()
	await _pause(1.6)
	var dungeon: DungeonController = current_scene as DungeonController
	var melee: Array[BasicEnemy] = _basic_enemies(dungeon)
	var shipped: bool = melee.size() == 5 and melee.all(func(e: BasicEnemy) -> bool:
		return (e.stats.resource_path.ends_with("basic_melee_enemy.tres") and e.get_state() == BasicEnemy.State.IDLE
			and not e.combat_enabled and e.support == null))
	_record(shipped and _supports_anywhere() == 0,
		"G%d) dungeon %d: the shipped five, all melee, parked, none a support; no support in it%s" % [
			run, run, " — nothing left of the first dungeon's" if run == 2 else ""])


# --- room one: the support's heal, for real -------------------------------------------------------------------

func _phase_room_one(run: int) -> void:
	var dungeon: DungeonController = current_scene as DungeonController
	var p: Player = dungeon.get_player()
	var room: RoomController = dungeon.get_rooms()[0]
	var melee: Array[BasicEnemy] = _room_enemies(room)
	var support: BasicEnemy = _spawn(dungeon, ROOM_ONE_SUPPORT)
	var fresh: bool = support.health_component.current_health == 65.0 and support.get_attack_phase() == EnemyAttack.Phase.NONE \
		and not support.support.has_plan() and support.support.get_heal_count() == 0 \
		and support.support.get_heal_cooldown_remaining() == 0.0 and support.support.get_allies().is_empty()
	var listeners: Array = [support.state_changed.get_connections().size(),
		support.health_component.died.get_connections().size(),
		support.health_component.damaged.get_connections().size(),
		support.support.support_applied.get_connections().size(),
		(support.attack as EnemyRangedAttack).fired.get_connections().size()]
	var same: bool = true
	if run == 1:
		_first_listeners = listeners
	else:
		same = listeners == _first_listeners
	p.hurtbox.set_invulnerable(true)
	p.health_component.heal(p.health_component.max_health)
	p.global_position = ROOM_ANCHORS[0]
	var shadow: BasicMeleeShadow = p.shadow_summoner.get_active_node()
	if shadow != null:
		shadow.hurtbox.set_invulnerable(true)
	var armed: bool = await _until(func() -> bool:
		p.global_position = ROOM_ANCHORS[0]
		return room.get_state() == RoomController.RoomState.ACTIVE, 3.0)
	# One of the room's melee brought to 40%: the support heals it.
	var hurt: BasicEnemy = melee[0]
	hurt.health_component.current_health = 40.0
	var first: int = _applied.size()
	var healed: bool = await _until(func() -> bool:
		p.global_position = ROOM_ANCHORS[0]
		return _heals_on(hurt, first) >= 1, 12.0)
	var bar: EnemyHealthBar3D = hurt.get_node("EnemyHealthBar3D") as EnemyHealthBar3D
	var heal: Dictionary = _applied[-1] if healed else {}
	_record(fresh and same and armed and healed and heal["amount"] == 25.0 and hurt.health_component.current_health == 65.0
			and is_equal_approx(bar.get_ratio(), 0.65) and support.get_target() == p,
		"R%d.1) room one, a support beside its melee: one melee brought to 40%%, the support heals it — 40 -> 65, a quarter of its 100 — through its health, its bar redrawn%s" % [
			run, "; fresh, its listeners as in the first dungeon" if run == 2 else ""])

	if run == 1:
		# Hurt again: the next heal is cut off by the player's heavy.
		support.health_component.set_max_health(1000.0)
		support.health_component.current_health = 1000.0
		hurt.health_component.current_health = 40.0
		var casting: bool = await _until(func() -> bool:
			p.global_position = ROOM_ANCHORS[0]
			return support.support.is_casting() and support.support.get_support_target() == hurt, 12.0)
		first = _applied.size()
		await _heavy(p, support)
		var cut: bool = support.is_staggered() or support.support.get_heal_cooldown_remaining() > 0.0
		await _pause(1.0)
		_record(casting and cut and _heals_on(hurt, first) == 0 and hurt.health_component.current_health <= 40.0,
			"R1.2) hurt again, the support starts the next heal — the player reaches it, and a heavy cuts the cast off: nothing healed")

	# The room's melee killed: the room clears, the support — not the room's —
	# changing nothing of it.
	support.health_component.set_max_health(1000.0)
	support.health_component.current_health = 1000.0
	for e in melee:
		await _kill(p, e)
	await _pause(0.5)
	_record(room.is_cleared(), "R%d.3) the melee killed: the room clears — the support, not the room's, changing nothing of it" % run)

	# Locked on, and brought down.
	p.global_position = ROOM_ANCHORS[0]
	p.camera_rig.rotation.y = 0.0
	await _frames(2)
	_press(p, &"target_lock")
	var locked: bool = p.targeting.get_target() == support
	var xp: int = p.progression.get_total_xp()
	var deaths: Array[int] = [0]
	support.enemy_died.connect(func(_dead: RoomCombatant) -> void: deaths[0] += 1)
	support.health_component.current_health = 50.0
	_no_crits.critical_chance = 1.0
	var tries: int = 0
	while not support.has_died() and tries < 4:
		await _heavy(p, support)
		tries += 1
	_no_crits.critical_chance = 0.0
	await _pause(0.4)
	_record(locked and support.has_died() and deaths[0] == 1 and p.progression.get_total_xp() - xp == 30
			and not p.targeting.is_locked() and support.support.get_support_target() == null,
		"R%d.4) locked on the support, a critical heavy brings it down — 30 XP once, nothing for its heals, the lock let go" % run)
	if shadow != null and is_instance_valid(shadow):
		shadow.hurtbox.set_invulnerable(false)


# --- room two: the shadow's kill ------------------------------------------------------------------------

func _phase_room_two() -> void:
	var dungeon: DungeonController = current_scene as DungeonController
	var p: Player = dungeon.get_player()
	var shadow: BasicMeleeShadow = p.shadow_summoner.get_active_node()
	var room: RoomController = dungeon.get_rooms()[1]
	var melee: Array[BasicEnemy] = _room_enemies(room)
	p.global_position = ROOM_ANCHORS[1]
	shadow.global_position = ROOM_ANCHORS[1] + Vector3(1.5, 0, 0)
	var armed: bool = await _until(func() -> bool: return room.get_state() == RoomController.RoomState.ACTIVE, 2.0)
	var support: BasicEnemy = _spawn(dungeon, ROOM_TWO_SUPPORT)
	await _frames(3)
	support.health_component.current_health = 10.0
	var player_xp: int = p.progression.get_total_xp()
	var shadow_xp: int = _shadow_total_xp(shadow.instance)
	p.shadow_commander.set_mode(BasicMeleeShadow.CommandMode.AGGRESSIVE)
	shadow.set_manual_target(support)
	var killed: bool = await _until(func() -> bool:
		p.global_position = ROOM_ANCHORS[1]
		return support.has_died(), 15.0)
	await _pause(0.3)
	var reward: int = support.get_xp_reward()
	var cut: int = int(round(reward * PlayerProgression.SHADOW_KILL_SHARE))
	var player_gain: int = p.progression.get_total_xp() - player_xp
	var shadow_gain: int = _shadow_total_xp(shadow.instance) - shadow_xp
	_record(armed and killed and support.get_killer() == shadow and shadow_gain == cut and player_gain == reward - cut,
		"S1) the shadow catches a support and kills it: its kill, split 70/30 — shadow +%d, player +%d of its %d" % [
			shadow_gain, player_gain, reward])
	p.shadow_commander.set_mode(BasicMeleeShadow.CommandMode.FOLLOW)
	p.hurtbox.set_invulnerable(true)
	for e in melee:
		await _kill(p, e)
	await _pause(0.5)
	_record(room.is_cleared(), "S2) room two's melee killed: the room clears")


# --- the boss, and leaving mid-cast -----------------------------------------------------------------------------

func _phase_boss() -> void:
	var dungeon: DungeonController = current_scene as DungeonController
	var p: Player = dungeon.get_player()
	p.global_position = ROOM_ANCHORS[2]
	var boss: DungeonBoss = _boss(dungeon)
	var awake: bool = await _until(func() -> bool: return boss.combat_enabled, 6.0)
	# A support beside the hurt boss: the boss runs its own AI — no EnemyAttack —
	# and is never its ally, never healed.
	p.hurtbox.set_invulnerable(true)
	boss.health_component.current_health = 40.0
	var support: BasicEnemy = _spawn(dungeon, boss.global_position + Vector3(4, 0, 0))
	var first: int = _applied.size()
	await _until(func() -> bool:
		_stick(p, boss)
		return support.support.get_scan_count() >= 3, 4.0)
	var ignored: bool = support.support.get_scan_count() >= 3 and not support.support.get_allies().has(boss) \
		and _applied.size() == first and boss.health_component.current_health == 40.0
	_stick(p, boss)
	await _frames(2)
	_press(p, &"target_lock")
	var locked: bool = p.targeting.get_target() == boss
	await _heavy(p, boss)
	await _pause(0.6)
	_record(awake and ignored and locked and boss.has_died() and not p.targeting.is_locked()
			and dungeon.get_state() == DungeonController.DungeonState.COMPLETED,
		"B1) the boss, on its own AI: a support beside it at 40 HP never counts it an ally nor heals it; locked and killed, the dungeon completes")


func _phase_out_mid_cast() -> void:
	var dungeon: DungeonController = current_scene as DungeonController
	var p: Player = dungeon.get_player()
	p.global_position = EXIT_POS
	var ally: BasicEnemy = MELEE_SCENE.instantiate() as BasicEnemy
	ally.position = EXIT_POS + Vector3(2, 0, 4)
	dungeon.add_child(ally)
	ally.set_combat_enabled(true)
	ally.health_component.current_health = 40.0
	var support: BasicEnemy = _spawn(dungeon, EXIT_POS + Vector3(-2, 0, 7))
	var casting: bool = await _until(func() -> bool:
		p.global_position = EXIT_POS
		return support.support.is_casting(), 6.0)
	dungeon.exit_portal.activate()
	await _pause(1.6)
	_record(casting and not is_instance_valid(support) and not is_instance_valid(ally) and current_scene.scene_file_path == HUB
			and _supports_anywhere() == 0 and int(Performance.get_monitor(Performance.OBJECT_ORPHAN_NODE_COUNT)) == 0,
		"O1) leaving through the exit while a support casts a heal: it goes with the dungeon — no cast, plan, marker or target of it survives, nothing orphaned")


# --- helpers ------------------------------------------------------------------------------------------------

func _spawn(dungeon: DungeonController, at: Vector3) -> BasicEnemy:
	var enemy: BasicEnemy = SUPPORT_SCENE.instantiate() as BasicEnemy
	enemy.position = at
	dungeon.add_child(enemy)
	var support_name: String = String(enemy.name)
	enemy.support.support_applied.connect(func(ally: RoomCombatant, action: AttackData, amount: float) -> void:
		_applied.append({"support": support_name, "ally": ally, "action": action, "amount": amount}))
	enemy.set_combat_enabled(true)
	return enemy


func _heals_on(ally: BasicEnemy, from: int) -> int:
	return _applied.slice(from).filter(func(a: Dictionary) -> bool:
		return is_same(a["ally"], ally) and a["action"] == HEAL).size()


func _supports_anywhere() -> int:
	var n: int = 0
	for node in root.find_children("*", "BasicEnemy", true, false):
		var enemy: BasicEnemy = node as BasicEnemy
		if enemy != null and enemy.support != null:
			n += 1
	return n


func _basic_enemies(dungeon: DungeonController) -> Array[BasicEnemy]:
	var out: Array[BasicEnemy] = []
	for room in dungeon.get_rooms():
		out.append_array(_room_enemies(room))
	return out


func _room_enemies(room: RoomController) -> Array[BasicEnemy]:
	var out: Array[BasicEnemy] = []
	for combatant in room.get_enemies():
		var enemy: BasicEnemy = combatant as BasicEnemy
		if enemy != null:
			out.append(enemy)
	return out


func _boss(dungeon: DungeonController) -> DungeonBoss:
	for combatant in dungeon.get_rooms()[2].get_enemies():
		if combatant is DungeonBoss:
			return combatant as DungeonBoss
	return null


## The player's light attack at `target` until it dies, standing in front of it.
func _kill(p: Player, target: RoomCombatant, budget: float = 30.0) -> void:
	var elapsed: float = 0.0
	while elapsed < budget and not target.has_died():
		if p.combat.get_state() == PlayerCombat.State.IDLE:
			p.global_position = target.global_position + Vector3(0, 0, STRIKE_RANGE)
			p.camera_rig.rotation.y = 0.0
			p.camera_rig.attack_light_pressed.emit()
		await physics_frame
		elapsed += DT


## Bounded: a killing blow on the boss opens the run summary, which pauses the
## tree with the swing still in its recovery.
func _heavy(p: Player, glue: Node3D) -> void:
	p.combat.reset()
	_stick(p, glue)
	p.camera_rig.rotation.y = 0.0
	p.camera_rig.attack_heavy_pressed.emit()
	var frames: int = 0
	while p.combat.is_attacking() and frames < 300 and not paused:
		await physics_frame
		frames += 1
		_stick(p, glue)


func _stick(p: Player, glue: Node3D) -> void:
	if glue != null and is_instance_valid(glue):
		p.global_position = glue.global_position + Vector3(0, 0, STRIKE_RANGE)


func _press(p: Player, action: StringName) -> void:
	var event: InputEventAction = InputEventAction.new()
	event.action = action
	event.pressed = true
	p._unhandled_input(event)


func _until(condition: Callable, budget: float = 3.0) -> bool:
	var waited: float = 0.0
	while not condition.call():
		if waited >= budget:
			return false
		await physics_frame
		waited += DT
	return true


func _frames(n: int) -> void:
	for i in n:
		await physics_frame


func _shadow_total_xp(shadow: ShadowInstance) -> int:
	var total: int = shadow.current_xp
	for level in range(1, shadow.level):
		total += shadow.shadow_data.xp_required_for_level(level)
	return total


func _pause(t: float) -> void:
	RunSummary.dismiss_open(self)
	await create_timer(t).timeout


func _record(passed: bool, description: String) -> void:
	if passed:
		_pass += 1
		print("[PASS] " + description)
	else:
		_fail += 1
		print("[FAIL] " + description)
